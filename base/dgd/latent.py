...
class GaussianMixtureSupervised(GaussianMixture):
    ...
    def forward(self, x, label=None):
        ...
        # Main computation
        y = - (x.unsqueeze(-2).unsqueeze(-2) - self.mean.to(device).view(self.Nclass, self.Ncpc, -1)
            ).square().div(2 * self.covariance.to(device).view(self.Nclass, self.Ncpc, -1)).sum(-1)
        y = y + self._log_var_factor * self.log_var.to(device).view(self.Nclass, self.Ncpc, -1).sum(-1)
        y = y + self._pi_term
        y += torch.log_softmax(self.weight.to(device).view(self.Nclass, self.Ncpc), dim=-1)
        y = y.sum(-1)
        
        y = y[(torch.arange(y.size(0), device=device), label)] * self.Nclass
